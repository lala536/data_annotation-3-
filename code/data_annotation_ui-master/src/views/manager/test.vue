    ...
    <el-container style="height: 89vh">
      ...
      <el-main style="box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1); width: 66%">
        ...
        <div style="margin-bottom: 10px;">
          ...
          <el-select v-model="selectedPersonId" @change="showRecord" clearable style="width: 70px;">
            <el-option
              :label="'全选'"
              :value="'全选'">
            </el-option>
          ...
          <el-select v-model="selectedRect" @change="showRect" clearable style="width: 70px;">
            <el-option
              :label="'全选'"
              :value="'全选'">
            </el-option>
          ...
          <el-select v-model="selectedText" @change="showText" clearable style="width: 70px;">
            <el-option
              :label="'全选'"
              :value="'全选'">
            </el-option>
          ...
          <el-dialog title="标签" :visible.sync="labelTypeDialog" width="50%" @close="closeLabelType">
            ...
            <el-radio-group v-model="selectedSingleLabelType" style="display: flex; flex-direction: column;">
      ...
      <el-main style="box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1); width: 34%">
        <!-- 导出 -->
        <el-button type="primary" plain @click="exportRecord" style="margin-bottom: 10px;">导出</el-button>

        <el-button type="primary" plain @click="annotationText">标注文本测试按钮</el-button>
        <!-- 表格 -->
        ...
        <el-table v-else :data="textData" height="580" :default-sort = "{prop: 'start', order: 'ascending'}" border :highlight-current-row="true" style="width: 100%;">
          ...
          <el-table-column width="250px" prop="text" label="text">
            <template slot-scope="scope">
              <span v-if="isEditingTextRow !== scope.$index">
...
export default {
  data() {
    return {
      ...
      usernames: [],
      showSelectAnnotator:false,
      selectedUsername: null,
      firstSelectedUsername: null,
      //////////////////////////选择视频并播放//////////////////////////
      videoDrawer: false,
      ...
      selectedSingleLabelType: null,
      selectedMultiLabelType: null,
      finalSelectedSingleLabelType: null,
      ...
      textData: [],
      textSegments: [],
      isEditingTextRow: null,
      annotationTextInProgress: null,
      firstTextStart: null,
      firstTextEnd: null,
      firstText: null,
    }
  ...
  mounted() {
    ...
    // 监听画框操作
    document.addEventListener('keydown', this.drawKeyDown)
  },
  methods: {
    ///////////////// 选择标注者 /////////////////
    // 加载所有user姓名
    ...
    annotation(label) {
      if (this.labelTypeMap[label] === this.finalSelectedMultiLabelType && this.selectedPersonId === '全选') {
      ...
      } else if (this.labelTypeMap[label] === this.finalSelectedSingleLabelType && this.selectedPersonId !== '全选') {
        if (!this.selectedPersonId) {
          this.$message.error("请选择被标注者")
        } else if (this.isEditingRow !== null) {
          this.$message.error("请先完成修改")
        ...
        this.$message.error("请点击对应的标签组按钮")
      }
    },
    ...
    deleteRect(event) {
      if (event.key === 's' && this.canvas[this.selectedVideoIndex].getActiveObject()) {
        ...
        for (let i = 1; i <= this.selectedVideos.length; i++) {
          for (let [index, item] of this.faces[i].entries()) {
            if (item.personId === selectedObject.personId && item.time <= this.formatTooltip(this.currentTime)) {
              if (!closestFace || item.time >= closestFace.time) {
                closestFace = item
              }
            }
          }
    ...
    exportRecord() {  // 导出数据
      if (this.isEditingRow !== null) {
        ...
        if (this.selectedPersonId === '全选') {
          ...
          // 构建导出的URL
          const exportURL = "http://localhost:8888/groupRecord/exportGroupRecord?" + Object.entries(params).map(([key, value]) => key + '=' + value).join('&')
